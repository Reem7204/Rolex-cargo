...
<?php
    if(isset($_POST['update'])){
        ...
        $sql2="UPDATE `cargotype` SET `name` = '$c1 ', `description` = '$d1', `cost` = '$c2' WHERE `cargotype`.`cargo_id` = $id";
        $result2=mysqli_query($con,$sql2);
        echo $result2;
        if($result2){
            echo "<script>alert('Updated successfully');window.location='viewcargotype.php'</script>";	
        }
    }
?>
...
<form action="" method="post">
    ...
    <table>
        ...
        <th><input type="text" name="description" value="<?php echo $des; ?>" required></th></tr>

        <tr><th>Amount</th>
        <th><input type="text" name="amount" value="<?php echo $c; ?>" required></th></tr>
        </table>
